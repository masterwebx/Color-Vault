...
import time
import platform
from utils import format_color_for_as3, color_to_int, int_to_color_str, resource_path, logger
...
class AddCostumeWindow(tk.Toplevel):
    def __init__(self, main_app, character, on_save_callback=None, costume_data=None, image_path=None, source=None, current_idx=None, loaded_idx=None, theme=None, style=None):
        super().__init__()
        ...
        self.theme = theme or {"fg": "black", "bg": "#d9d9d9", "button_fg": "black", "button_bg": "#f0f0f0", "canvas_bg": "#333333"}
        self.style = style  # ttkbootstrap Style object
        self.theme_transparency = self.main_app.theme_transparency  # Still a reference to the main app's theme_transparency (now a BooleanVar)
        self._setup_window()
    ...
    def _setup_window(self):
        ...
        try:
            self.wm_iconbitmap(resource_path("icon.ico"))  # Updated to use utils.resource_path
        except tk.TclError as e:
            logger.warning(f"Failed to set AddCostumeWindow icon: {e}")
    ...
    def _create_ui(self):
        ...
        left_panel = tk.Frame(main_frame, bg=self.theme["bg"])
        left_panel.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=5)

        tk.Label(left_panel, text="New Costume JSON:", fg=self.theme["fg"], bg=self.theme["bg"]).pack(pady=5)
        self.new_costume_text = scrolledtext.ScrolledText(left_panel, height=20, width=50, fg=self.theme["fg"], bg=self.theme["bg"])
        self.new_costume_text.pack(padx=5, pady=5, fill=tk.BOTH, expand=True)
        self._populate_json_text()

        self.button_frame = tk.Frame(left_panel, bg=self.theme["bg"])
        self.button_frame.pack(pady=5, fill=tk.X)

        right_panel = tk.Frame(main_frame, bg=self.theme["bg"])
        ...
        # Ensure all child widgets are updated with the theme
        self.main_app.update_widget_colors(self, self.theme)

    def _create_image_previews(self, parent):
        image_previews_frame = tk.Frame(parent, bg=self.theme["bg"])
        image_previews_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=5)

        tk.Label(image_previews_frame, text="Uploaded Image Preview", font=("Arial", 12), fg=self.theme["fg"], bg=self.theme["bg"]).pack(pady=5)
        self.uploaded_preview_canvas = tk.Canvas(image_previews_frame, width=300, height=200, bg=self.theme["canvas_bg"], highlightthickness=2, highlightbackground=self.theme["fg"])
        self.uploaded_preview_canvas.pack(pady=5)
        self.zoom_var = tk.DoubleVar(value=0.0)
        zoom_frame = tk.Frame(image_previews_frame, bg=self.theme["bg"])
        zoom_frame.pack(pady=5)
        tk.Label(zoom_frame, text="Zoom:", fg=self.theme["fg"], bg=self.theme["bg"]).pack(side=tk.LEFT)
        self.zoom_slider = ttk.Scale(zoom_frame, from_=-2.0, to=2.0, orient=tk.HORIZONTAL, variable=self.zoom_var, command=lambda _: self.update_uploaded_preview(), style="TScale")
        self.zoom_slider.pack(side=tk.LEFT, fill=tk.X, expand=True)
        self.uploaded_preview_label = tk.Label(image_previews_frame, text="Upload an image to see the preview", fg=self.theme["fg"], bg=self.theme["bg"])
        self.uploaded_preview_label.pack(pady=5)

        tk.Label(image_previews_frame, text="Recolor Preview", font=("Arial", 12), fg=self.theme["fg"], bg=self.theme["bg"]).pack(pady=5)
        self.recolor_preview_canvas = tk.Canvas(image_previews_frame, width=300, height=200, bg=self.theme["canvas_bg"], highlightthickness=2, highlightbackground=self.theme["fg"])
        self.recolor_preview_canvas.pack(pady=5)
        self.recolor_zoom_var = tk.DoubleVar(value=0.0)
        recolor_zoom_frame = tk.Frame(image_previews_frame, bg=self.theme["bg"])
        recolor_zoom_frame.pack(pady=5)
        tk.Label(recolor_zoom_frame, text="Recolor Zoom:", fg=self.theme["fg"], bg=self.theme["bg"]).pack(side=tk.LEFT)
        self.recolor_zoom_slider = ttk.Scale(recolor_zoom_frame, from_=-2.0, to=2.0, orient=tk.HORIZONTAL, variable=self.recolor_zoom_var, command=lambda _: self.update_recolor_preview(), style="TScale")
        self.recolor_zoom_slider.pack(side=tk.LEFT, fill=tk.X, expand=True)
        self.recolor_preview_label = tk.Label(image_previews_frame, text="Select a character and upload an image to see the recolor", fg=self.theme["fg"], bg=self.theme["bg"])
        self.recolor_preview_label.pack(pady=5)

    def _create_palette_previews(self, parent):
        palette_previews_frame = tk.Frame(parent, bg=self.theme["bg"])
        palette_previews_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=5)

        tk.Label(palette_previews_frame, text="Extracted Palette Strips", font=("Arial", 12), fg=self.theme["fg"], bg=self.theme["bg"]).pack(pady=5)
        self.extracted_palette_canvas = tk.Canvas(palette_previews_frame, width=300, height=200, bg=self.theme["canvas_bg"], highlightthickness=2, highlightbackground=self.theme["fg"])
        self.extracted_palette_canvas.pack(pady=5)
        self.extracted_palette_label = tk.Label(palette_previews_frame, text="Upload an image to see the extracted palettes", fg=self.theme["fg"], bg=self.theme["bg"])
        self.extracted_palette_label.pack(pady=5)

        tk.Label(palette_previews_frame, text="Converted Palette Strips", font=("Arial", 12), fg=self.theme["fg"], bg=self.theme["bg"]).pack(pady=5)
        self.converted_palette_canvas = tk.Canvas(palette_previews_frame, width=300, height=200, bg=self.theme["canvas_bg"], highlightthickness=2, highlightbackground=self.theme["fg"])
        self.converted_palette_canvas.pack(pady=5)
        self.converted_palette_label = tk.Label(palette_previews_frame, text="Edit JSON data to see converted palettes", fg=self.theme["fg"], bg=self.theme["bg"])
        self.converted_palette_label.pack(pady=5)

    def _create_buttons(self):
        tk.Button(self.button_frame, text="Upload Image", command=self.upload_image, fg=self.theme["button_fg"], bg=self.theme["button_bg"]).pack(side=tk.LEFT, padx=5)
        tk.Button(self.button_frame, text="Upload Recolor Sheet", command=self.upload_recolor_sheet, fg=self.theme["button_fg"], bg=self.theme["button_bg"]).pack(side=tk.LEFT, padx=5)
        tk.Button(self.button_frame, text="Save", command=self.prompt_for_costume_name, fg=self.theme["button_fg"], bg=self.theme["button_bg"]).pack(side=tk.LEFT, padx=5)
    ...
    def _update_json_with_palette_strips(self):
        if self.uploaded_image and self.original_palette_strips and self.uploaded_palette_strips:
            costume = {
                ...
                "paletteSwapPA": {
                    "colors": [int_to_color_str(c) for c in self.original_palette_strips[0]],
                    "replacements": [int_to_color_str(c) for c in self.uploaded_palette_strips[0]]
                },
                "paletteSwap": {
                    "colors": [int_to_color_str(c) for c in self.original_palette_strips[1]] if len(self.original_palette_strips) > 1 else [],
                    "replacements": [int_to_color_str(c) for c in self.uploaded_palette_strips[1]] if len(self.uploaded_palette_strips) > 1 else []
                }
            }
            self.new_costume_text.delete("1.0", tk.END)
            self.new_costume_text.insert(tk.END, json.dumps(costume, indent=2))
            logger.info("Updated JSON with palette strips from loaded/generated image")
            self._update_recolor_image()

    def _update_recolor_image(self):
        """Precompute the palette-swapped image based on current JSON and recolor sheet."""
        self.recolor_preview_label.config(text="Ready")
        ...
        try:
            ...
            logger.info("Precomputed palette-swapped recolor image")
        except Exception as e:
            logger.error(f"Error applying palette swap: {str(e)}")
            self.recolor_image = self.original_recolor_sheet.copy().convert("RGBA")
    ...
    def generate_image_from_preview(self):
        self.uploaded_preview_label.config(text="Ready")
        ...
        try:
            ...
            logger.info("Successfully generated and loaded image from preview")
    ...
    def load_recolor_sheet_preview(self):
        self.recolor_preview_label.config(text=f"Ready")
    ...
    def extract_palette_strips(self, img):
        ...
        # Collect colors from all detected strips to exclude them from ignored_colors
        detected_strip_colors = set()
        ...
        for y in range(height):
            ...
            # Exclude colors that are part of any detected strip
            ignored_colors -= detected_strip_colors
            # Add colors from adjacent rows, but only if they aren't part of a detected strip
            adjacent_colors = set()
            if y > 0:
                adjacent_colors.update(row_colors[y - 1])
            if y < height - 1:
                adjacent_colors.update(row_colors[y + 1])
            adjacent_colors -= detected_strip_colors
            ignored_colors.update(adjacent_colors)
            logger.info(f"Row {y}: Ignored colors: {[hex(c) for c in ignored_colors]}")
            ...
            while x < width:
                ...
                if color not in ignored_colors:
                    ...
                    while pos < width:
                        ...
                        color2 = 0 if a2 == 0 else ((a2 << 24) | (r2 << 16) | (g2 << 8) | b2)
                        if color2 not in ignored_colors:
                            strip_colors.append(color2)
                            pos += 1
                        else:
                            logger.info(f"Row {y}, x={pos}: Stopped at color {hex(color2)} (in ignored_colors)")
                            break
                    logger.info(f"Row {y}, x={x}: Strip colors: {[hex(c) for c in strip_colors]}")
                    if len(set(strip_colors) - {0}) >= 3:
                        if self._is_valid_strip(pixels, y, x, pos - 1, strip_colors, height):
                            ...
                            logger.info(f"Row {y}: Detected strip from x={start_x} to x={end_x}")
                            # Add strip colors to detected_strip_colors
                            detected_strip_colors.update(strip_colors)
                            break  # Found a strip, stop searching this row
                    x = pos
                else:
                    x += 1

            if has_valid_strip and y not in [row for row, _, _, _ in palette_rows]:
        ...
        for y, start_x, end_x, valid_strip_colors in palette_rows:
            ...
            for x in range(start_x, end_x + 1):
                r, g, b, a = pixels[x, y]
                if a == 0:
                    strip_colors.append(0)
                else:
                    color = (a << 24) | (r << 16) | (g << 8) | b
                    strip_colors.append(color)
            self.uploaded_palette_strips.append(strip_colors)
        logger.info(f"Detected palette strips at rows {[row for row, _, _, _ in palette_rows]}: {self.uploaded_palette_strips}")
    ...
    def update_uploaded_preview(self):
        ...
        if not self.uploaded_image:
            logger.info(f"No image loaded yet for preview")
            return
        try:
            ...
            logger.error(f"Error updating uploaded preview: {str(e)}")
    ...
    def update_recolor_preview(self):
        ...
        try:
            ...
            logger.error(f"Error updating recolor preview: {str(e)}")
    ...
    def _apply_palette_swap(self, image, costume):
        ...
        for orig, repl in zip(palette_swap["colors"], palette_swap["replacements"]):
            orig_int = self.main_app.convert_color(orig)
            repl_int = self.main_app.convert_color(repl)
    ...
    def update_extracted_palette_preview(self):
        self.extracted_palette_label.config(text="Ready")
    ...
    def update_converted_palette_preview(self):
        self.converted_palette_label.config(text="Ready")
    ...
    def _draw_palette_strips(self, draw, y_offset, row_height, strips, prefix):
        for i, strip in enumerate(strips):
            ...
            for x, color in enumerate(strip):
                if color == "transparent" or color == 0:  # Handle transparent colors
                    draw.rectangle([x * 10, y, x * 10 + 9, y + 9], fill=(0, 0, 0, 0))
                else:
                    # Assume color is an integer (e.g., 0xAARRGGBB)
                    r = (color >> 16) & 255
                    g = (color >> 8) & 255
                    b = color & 255
                    a = (color >> 24) & 255
    ...
    def prompt_for_costume_name(self):
        ...
        tk.Label(dialog, text="Costume Name:", fg=self.theme["fg"], bg=self.theme["bg"]).pack(pady=5)
        costume_name_entry = tk.Entry(dialog, fg=self.theme["fg"], bg=self.theme["bg"], insertbackground=self.theme["fg"])
        costume_name_entry.pack(pady=5)

        tk.Label(dialog, text="Online Name:", fg=self.theme["fg"], bg=self.theme["bg"]).pack(pady=5)
        online_name_entry = tk.Entry(dialog, fg=self.theme["fg"], bg=self.theme["bg"], insertbackground=self.theme["fg"])
        online_name_entry.insert(0, self.online_name)
        online_name_entry.pack(pady=5)

        tk.Label(dialog, text="Note: Names must be JSON-friendly.\nAvoid special characters like quotes and braces.", fg=self.theme["fg"], bg=self.theme["bg"]).pack(pady=5)
        tk.Button(dialog, text="Save", command=lambda: self._save_costume_from_dialog(costume_name_entry, online_name_entry, dialog), fg=self.theme["button_fg"], bg=self.theme["button_bg"]).pack(pady=10)

        # Apply theme to all widgets in the dialog
        self.main_app.update_widget_colors(dialog, self.theme)
    ...
    def save_new_costume(self, costume_name, online_name):
        try:
            ...
            logger.info(f"Saving costume: {json.dumps(costume, indent=2)}")
